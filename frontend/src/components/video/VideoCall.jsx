...
export default function VideoCall({ role = "mentor" }) {
    ...
    // Move ICE candidate queue outside of useEffect to make it persistent
    const iceCandidateQueue = useRef([]);
    // Track if we need to send an offer when learner connects
    ...
    useEffect(() => {
        ...
        newSocket.onopen = () => {
            ...
            // If mentor has a pending offer, send it now
            if (pendingOfferRef.current && role === "mentor") {
                socketRef.current.send(JSON.stringify(pendingOfferRef.current));
        ...
        newSocket.onmessage = async (event) => {
            ...
            // Handle participant joined - send offer if mentor and has one ready
            if (data.type === "participant-joined" && role === "mentor" && peerConnection.current && localStream) {
                try {
                    ...
                    console.log("📨 Sent SDP offer to newly joined participant");
                } catch (error) {
    ...
    const startCall = async () => {
        try {
            ...
            // Notify server that participant has joined
            ...
            // If mentor, create and send offer immediately if WebSocket is open
            if (role === "mentor") {
                try {
                    const offer = await peerConnection.current.createOffer();
                    await peerConnection.current.setLocalDescription(offer);
                    ...
                    if (socketRef.current?.readyState === WebSocket.OPEN) {
                        ...
                        console.log("📨 Sent SDP offer immediately");
                    } else {
                        // Store offer to send when WebSocket opens
                        pendingOfferRef.current = offerMessage;
                        console.log("🕓 Stored pending offer for when WebSocket opens");
                    }
                } catch (error) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 max-w-6xl mx-auto mb-8">
                ...
                <div className="relative group">
                    ...
                    <div className="relative overflow-hidden rounded-2xl shadow-2xl">
                        ...
                        />
                        {callStatus !== "In Call" && (
                            <div className="absolute inset0 flex items-center justify-center bg-gray-800 bg-opacity-80 backdrop-blur-sm">
                                <div className="text-center text-white">
                                    <div className="text-4xl mb-2">👤</div>
                ...
                <div className="flex flex-wrap justify-center items-center gap-3 sm:gap-4 p-4 bg-white bg-opacity-10 backdrop-blur-sm rounded-2xl border border-white border-opacity-20">
                    ...
                    {!callStarted ? (
                        ...
                        <>
                            ...
                            <button
                                onClick={toggleMic}
                                className={`px-4 py-3 rounded-xl text-sm font-medium shadow-md transition-all duration-300 hover:shadow-lg transform hover:scale-105 flex items-center gap-2 ${isMicOn
                            ...
                            <button
                                onClick={toggleCam}
                                className={`px-4 py-3 rounded-xl text-sm font-medium shadow-md transition-all duration-300 hover:shadow-lg transform hover:scale-105 flex items-center gap-2 ${isCamOn
                                        ? 'bg-gray-200 hover:bg-gray-300 text-gray-800'
                                        : 'bg-red-500 hover:bg-red-600 text-white'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`px-4 py-3 rounded-xl text-sm font-medium shadow-md transition-all duration-300 hover:shadow-lg transform hover:scale-105 flex items-center gap-2 ${isScreenSharing
