...
#         if not current_user.is_authenticated:
#             logger.warning(f"Anonymous user attempted to connect to room {self.room_name}. Disconnecting.")
#             self.close(code=4001) # Unauthorized / Unauthenticated
#             return
...
#         try:
#             text_data_json = json.loads(text_data)
#             message_type = text_data_json.get('type') # NEW: Get message type (e.g., 'chat_message', 'typing_start')
...
#                 @database_sync_to_async
#                 def get_recipient_user(rec_id):
#                     try:
#                         return User.objects.get(id=rec_id)
#                     except User.DoesNotExist:
#                         return None

#                 recipient_user = async_to_sync(get_recipient_user)(recipient_id)
...
#                 @database_sync_to_async
#                 def create_chat_message(sender, recipient, room_name, content):
...
#                 new_message = async_to_sync(create_chat_message)(
#                     current_user,
#                     recipient_user,
#                     self.room_name,
#                     message_content
...
#                 logger.info(f"Received typing event '{message_type}' from {current_user.email} in room {self.room_name}")
#                 # Broadcast typing status to the group, excluding the sender
#                 async_to_sync(self.channel_layer.group_send)(
#                     self.room_group_name,
#                     {
#                         'type': 'typing_status', # This will call the typing_status method
#                         'sender_id': current_user.id,
#                         'sender_email': current_user.email,
#                         'is_typing': (message_type == 'typing_start'), # True for start, False for stop
#                     }
#                 )
#             else:
#                 logger.warning(f"Received unknown message type: {message_type} from {current_user.email}")
...
#         self.send(text_data=json.dumps({
#             'type': 'chat_message', # NEW: Add type for frontend to distinguish
#             'message': event['message'],
#             'sender_id': event['sender_id'],
#             'sender_email': event['sender_email'],
#             'recipient_id': event['recipient_id'],
...
#     # NEW: Receive typing status from room group
#     def typing_status(self, event):
#         # Only send typing status if it's from another user
#         if self.scope["user"].id != event['sender_id']:
#             self.send(text_data=json.dumps({
#                 'type': 'typing_status', # NEW: Add type for frontend to distinguish
#                 'sender_id': event['sender_id'],
#                 'sender_email': event['sender_email'],
#                 'is_typing': event['is_typing'],
#             }))
